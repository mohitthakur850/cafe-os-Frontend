      ...
      {errorMessage && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(8px)', zIndex: 9999, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
          <div className="shake-animation" style={{ backgroundColor: 'white', padding: '40px', borderRadius: '24px', maxWidth: '500px', width: '90%', textAlign: 'center', boxShadow: '0 20px 50px rgba(0,0,0,0.2)' }}>
            <div style={{ fontSize: '4rem', marginBottom: '15px' }}>⚠️</div><h2 style={{ color: '#222', fontSize: '2rem', margin: '0 0 15px 0' }}>Oops!</h2><p style={{ color: '#555', fontSize: '1.3rem', lineHeight: '1.5', margin: '0 0 30px 0' }}>{errorMessage}</p>
            <button onClick={() => setErrorMessage('')} style={{ padding: '15px 40px', backgroundColor: '#ef4444', color: 'white', border: 'none', borderRadius: '50px', fontSize: '1.3rem', fontWeight: 'bold', cursor: 'pointer' }}>Okay, Got it!</button>
          </div>
        </div>
      )}
          ...
          <div style={{ width: '280px', backgroundColor: 'white', borderRight: '1px solid #eaeaea', display: 'flex', flexDirection: 'column', zIndex: 10 }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '20px 15px' }}>
              {dynamicCategories.map(cat => {
                ...
                return (<div key={cat.main} className={`sidebar-btn ${isActive ? 'active' : ''}`} onClick={() => { setActiveCategory(cat.main); setActiveSubCategory('All'); }} style={{ display: 'flex', alignItems: 'center', padding: '12px', marginBottom: '12px', borderRadius: '18px', cursor: 'pointer', border: '1px solid #eee' }}><img src={cat.image} alt={cat.main} style={{ width: '45px', height: '45px', borderRadius: '12px', objectFit: 'cover' }} /><span style={{ marginLeft: '15px', fontSize: '1.2rem', fontWeight: 'bold', color: isActive ? '#222' : '#555' }}>{cat.main}</span></div>);
              })}
            </div>
          </div>
          <div style={{ flex: 1, overflowY: 'auto', backgroundColor: '#fcfcfc', display: 'flex', flexDirection: 'column' }}>
            {currentCategoryData && (<div key={activeCategory + '-banner'} className="fade-in" style={{ width: '100%', height: '240px', position: 'relative', flexShrink: 0 }}><img src={currentCategoryData.image} alt={activeCategory} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /><div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(0,0,0,0.8) 0%, rgba(0,0,0,0.1) 100%)' }}></div><h1 style={{ position: 'absolute', bottom: '25px', left: '40px', margin: 0, fontSize: '4rem', color: 'white', fontWeight: '900', letterSpacing: '-1px' }}>{activeCategory.toUpperCase()}</h1></div>)}
            <div style={{ padding: '40px' }}>
              {currentCategoryData && currentCategoryData.subs.length > 0 && (<div style={{ display: 'flex', gap: '12px', overflowX: 'auto', marginBottom: '40px', paddingBottom: '10px' }}><div className="sub-cat-chip" onClick={() => setActiveSubCategory('All')} style={{ padding: '10px 25px', borderRadius: '30px', cursor: 'pointer', fontSize: '1.1rem', fontWeight: 'bold', backgroundColor: activeSubCategory === 'All' ? '#333' : 'white', color: activeSubCategory === 'All' ? 'white' : '#555', border: activeSubCategory === 'All' ? '2px solid #333' : '2px solid #eaeaea' }}>All</div>{currentCategoryData.subs.map(sub => (<div key={sub} className="sub-cat-chip" onClick={() => setActiveSubCategory(sub)} style={{ padding: '10px 25px', borderRadius: '30px', cursor: 'pointer', fontSize: '1.1rem', fontWeight: 'bold', backgroundColor: activeSubCategory === sub ? '#333' : 'white', color: activeSubCategory === sub ? 'white' : '#555', border: activeSubCategory === sub ? '2px solid #333' : '2px solid #eaeaea' }}>{sub}</div>))}</div>)}
              <div key={activeCategory + activeSubCategory} className="animated-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '30px' }}>
                {displayProducts.length === 0 ? (<p style={{ color: '#888', fontSize: '1.2rem', marginTop: '20px', gridColumn: '1 / -1' }}>No items found.</p>) : (
                  displayProducts.map(product => {
                    const qtyInCart = getProductTotalQty(product); const isAvailable = product.isAvailable !== false;
                    return (
                      <div key={product._id || product.id} className="product-card" style={{ backgroundColor: 'white', borderRadius: '24px', padding: '15px', boxShadow: '0 6px 16px rgba(0,0,0,0.04)', border: '1px solid #f0f0f0', display: 'flex', flexDirection: 'column', opacity: isAvailable ? 1 : 0.6, pointerEvents: isAvailable ? 'auto' : 'none', filter: isAvailable ? 'none' : 'grayscale(100%)' }}>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(5px)', zIndex: 1000, display: 'flex', justifyContent: 'center', alignItems: 'flex-end' }}>
          <div style={{ width: '500px', backgroundColor: 'white', borderTopLeftRadius: '35px', borderTopRightRadius: '35px', padding: '40px', boxSizing: 'border-box', animation: 'slideUpFade 0.3s forwards' }}>
            ...
            <div style={{ marginBottom: '35px', maxHeight: '250px', overflowY: 'auto' }}>
              {selectedProduct.addons && selectedProduct.addons.length > 0 ? (selectedProduct.addons.map(addon => { const isSelected = selectedAddons.includes(addon); return (<div key={addon.name} onClick={() => toggleAddon(addon)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '18px 20px', border: `2px solid ${isSelected ? '#28a745' : '#eaeaea'}`, borderRadius: '16px', marginBottom: '12px', cursor: 'pointer', backgroundColor: isSelected ? '#f2fdf5' : 'white', transition: 'all 0.2s' }}><span style={{ fontWeight: 'bold', fontSize: '1.1rem', color: isSelected ? '#1e7e34' : '#444' }}>{addon.name}</span><span style={{ color: isSelected ? '#1e7e34' : '#666', fontWeight: 'bold' }}>+₹{addon.price}</span></div>); })) : (<p style={{ color: '#888' }}>No extra add-ons available.</p>)}
            </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', marginBottom: '20px', paddingRight: '10px' }}>
              {cart.map((item) => (
                <div key={item.cartId} style={{ padding: '20px 0', borderBottom: '1px solid #eaeaea' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}><div style={{ flex: 1 }}><span style={{ fontSize: '1.3rem', fontWeight: 'bold', color: '#333' }}>{item.name}</span>{item.addons && item.addons.length > 0 && (<div style={{ color: '#888', fontSize: '0.9rem', marginTop: '4px' }}>+ {item.addons.map(a => a.name).join(', ')}</div>)}</div><div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}><span style={{ fontSize: '1.3rem', fontWeight: 'bold', color: '#222', marginBottom: '10px' }}>₹{item.itemTotal * item.quantity}</span><div style={{ display: 'flex', alignItems: 'center', backgroundColor: '#f1f3f5', borderRadius: '30px', padding: '5px' }}><button className="qty-btn" onClick={() => updateQuantity(item.cartId, -1)}>-</button><span style={{ margin: '0 15px', fontWeight: '900', fontSize: '1.2rem', color: '#333' }}>{item.quantity}</span><button className="qty-btn" onClick={() => updateQuantity(item.cartId, 1)}>+</button></div></div></div>
                </div>
              ))}
            </div>
